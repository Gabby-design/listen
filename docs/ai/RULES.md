...
**Statement:** The global rules marked "adopted" in ENGINEERING.md § 0 apply verbatim from ~/.agents/rules/.
**Canonical home:** `~/.agents/rules/<name>.md` (each) · **Mirrors:** ENGINEERING.md § 0 · **Enforced by:** review checklist · **Status:** active · **Origin:** owner, <date>
...
**Canonical home:** `ENGINEERING.md § n` · **Overrides:** `~/.agents/rules/<name>.md` · **Mirrors:** … · **Enforced by:** … · **Status:** active · **Origin:** owner, <date> (or AMD-nnn)
-->
...
**Statement:** A named skill, agent, workflow or rule is resolved from the project's vendored copy, then `~/.agents`, then the agent's own installed copy; if none resolves, say so and continue with docs/ai alone — never invent it.
...
**Statement:** From inside this repository every write stays inside it; the global folder (~/.agents and every path resolving into it) is read-only; global changes are proposed in the report and made only from a session opened in the global folder.
**Canonical home:** `AGENT-CORE.md § 8` (global statement: `~/.agents/GLOBAL.md § 2.8`)
**Mirrors:** ENGINEERING.md § 0 (overrides live here); generated pointer headers; runbooks
**Enforced by:** Claude Code scope-guard hook (PreToolUse); `agent new` / `agent caveman --global` refuse from a project; report lists proposed global changes
**Status:** active
...
**Enforced by:** `agent ctx status` clean before the report; derived layer listed in "Documentation synchronized"
**Status:** active
...
**Enforced by:** `agent doctor --project`
**Status:** active
